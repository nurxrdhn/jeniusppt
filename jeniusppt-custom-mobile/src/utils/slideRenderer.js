...
function addMediaPlaceholder(parent, element) {
  ...
  media.style.cssText =
    "width:100%;height:100%;display:grid;place-items:center;background:#111827;color:white;border-radius:12px;font:700 28px Arial;text-align:center;padding:16px";
  media.textContent =
    element.type === "audio"
      ? `🔊 ${element.fileName || "Audio"}`
      : `▶ ${element.fileName || "Video"}`;
...
export async function renderSlideToDataUrl(slide, slideSize) {
  ...
  const height = Math.round(
    width * ((slideSize?.height || 7.5) / (slideSize?.width || 13.333)),
  );
  ...
  if (slide.background?.type === "image") {
    ...
    root.style.background =
      slide.background?.value || "linear-gradient(135deg,#ff7a25,#e94d08)";

  const titleBox = box(slide.titleBox, { x: 8, y: 12, w: 84, h: 20 });
  const bodyBox = box(slide.bodyBox, { x: 8, y: 36, w: 84, h: 42 });
  const addText = (text, area, color, size, weight, formatting) => {
  ...
  for (const element of slide.elements || []) {
    ...
    if (element.type === "shape") {
      node.style.background = element.background || "#f97316";
    ...
    if (element.type === "video" || element.type === "audio")
      addMediaPlaceholder(node, element);
    root.appendChild(node);
  }
